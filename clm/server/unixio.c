...
// Send a command
int SendCommand(int sock, ClmCommand* command) {
  int i;
  command->serial = ClmSerialNumber;
  if (write_server_records) {
    ClmPrintCommand(command);
  }
  if(SendHeader(sock, command->command, command->serial, command->num_arg)
     == -1) {
    return -1;
  }
...
int ReceiveArgument(int sock, ClmArg* arg) {
  ...
  case ClmArgSymbol:
    ...
    // We use unsigned chars everwhere (hopefully) because it's easier
    // to assemble the other types. Most string functions, however
    // assume 'char' type, which could be signed or unsigned. So we
    // cast to that knowing that the length of a string irrespective
    // of whether the individual bytes are treated as signed or
    // unsigned is always the same. But note, it's not reliable to
    // treat arbitary binary data as a string.
...
ClmCommand *ReceiveCommand(int socket_fd) {
  ...
  cmd->args =
    (ClmArg*)ClmMalloc((cmd->num_arg == 0 ? 1 : cmd->num_arg) *
		       sizeof(ClmArg));
  for(i = 0; i < cmd->num_arg; ++i) {
    if (ReceiveArgument(socket_fd, &(cmd->args[i])) == -1) {
      return NULL;
    }
  }
